...
(function() {
  ...
  if (document.readyState == "complete") {
    ...
    function generateButton(element, title, icon) {
        ...
        element.innerHTML += `<button class="btn no-text btn-icon" id="waze-discuss-${id}-button" tabindex="-1" title="Insert ${title}" type="button">
<img src="${icon}" style="
    width: 1em;
    height: 1em;
">      <span aria-hidden="true">
        ...
        document.getElementById(`waze-discuss-${id}-button`).addEventListener("click", function (e) {appendIconToSignature(icon)});
    }
